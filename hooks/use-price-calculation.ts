...
// Vehicle type mapping for Olympic pricing
const mapVehicleTypeToOlympic = (type: string): keyof OlympicRoute['prices'] => {
  console.log(`🚗 MAPPING VEHICLE TYPE: "${type}"`)
  
  let mapped: keyof OlympicRoute['prices']
...
export function usePriceCalculation(state: BookingState, dispatch: (action: any) => void) {
  ...
  const mapToOlympicVehicleType = (standardType: string): keyof OlympicRoute['prices'] => {
    return mapVehicleTypeToOlympic(standardType)
  }
  ...
  const calculatePrice = useCallback(
    ...
    [isReadyForPricing, calculateOlympicPrice]
  )
